...
port: $Port
refreshSec: 30
docentWmUrl: http://127.0.0.1:$WmPort
configDir: $ConfigDir
...
if ($NoTasks) {
    ...
    $lnArgs = ('-NoLogo -NoProfile -File "{0}" -SessionsUrl "{1}" -WmUrl "http://127.0.0.1:{2}" -Hotkey "{3}"' -f $lnScript, $Sessions, $WmPort, $Hotkey)
...
Write-Host "    'wsm' directive whose base_url reaches this host's :$WmPort (e.g. over a reverse SSH"
